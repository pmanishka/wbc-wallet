...
(function($, window, document, undefined) {
	...
	function Plugin(element, options) {
		...
		if (this.init()) {
			...
			if (this.options.ReadQRCode || this.options.ReadBarcode) {
				this.setCallback();
			}
		}
	}
	Plugin.prototype = {
		...
		setEventListeners: function() {
			camera.addEventListener('canplay', function(e) {
				if (!isStreaming) {
					...
					isStreaming = true;
					if (Self.options.ReadQRCode || Self.options.ReadBarcode) {
						Self.delay();
					}
				}
			}, false);
		...
		cameraPlay: function(skip) {
			...
			setTimeout(function() {
				delay = false;
				if (Self.options.ReadBarcode) Self.tryParseBarcode();
